...
The vault is its own git repository, and a write leaves a commit: `write_card` writes `card <slug>: ADD|UPDATE|SUPERSEDE`, `write_file` inside the vault writes `file <path>: write`, the nightly rollup commits its own CORE.md edits, and the vault cleanup — whether an update runs it or you press 🧹 Vault cleanup in `/menu` — commits a pair: `… vault snapshot` for your uncommitted edits, then `… vault cleanup` for its own result. The commit names the paths it touched, so your own uncommitted edits in Obsidian are left alone — the next night's Brain pass picks them up. Commits go to the vault's own repository only: a vault that sits inside another repository is never committed into it. A `pre-commit` hook in that repository runs inside every memory commit, so it slows each write down, and a hanging hook holds the writes behind it. A commit never fails a turn: if git is not installed, the folder is not a repository, or the index is busy, the file is still written and the reason goes to the service journal in one line.
