...
test("повторный write_card сливает карточку: поля вне схемы, created и старый body живы", async () => {
  ...
  // Прежнее значение сменившегося description не выбрасывается: оно лежит в ## History
  // датированной строкой (смена факта без следа невозможна), а текущим стало новое —
  // исторический баг свёрнутых скаляров оставлял здесь именно старое.
  assert.equal(
    parseFrontmatter(out).fields?.description,
    "AI-контент криейтор, прислала новую смету",
  );
